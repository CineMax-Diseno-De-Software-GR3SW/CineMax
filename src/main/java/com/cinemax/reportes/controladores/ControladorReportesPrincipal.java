...
public class ControladorReportesPrincipal {
    ...
    @FXML private TableView<ReporteGenerado> tablaReportes;
    @FXML private TableColumn<ReporteGenerado, String> colNombre;
    @FXML private TableColumn<ReporteGenerado, String> colTipo;
    @FXML private TableColumn<ReporteGenerado, LocalDateTime> colFecha;
    @FXML private TableColumn<ReporteGenerado, String> colDescripcion;
    @FXML private TableColumn<ReporteGenerado, Integer> colAcciones;
    ...
    private final List<ReporteGenerado> reportesSimulados = Arrays.asList(
        new ReporteGenerado(1, "Reporte_Ventas_20241201_1430", "PDF", LocalDateTime.now().minusDays(2), "C:/reportes/reporte_ventas_20241201.pdf", "Reporte de ventas del 01/12/2024 al 05/12/2024"),
        new ReporteGenerado(2, "Reporte_Ventas_20241128_0915", "CSV", LocalDateTime.now().minusDays(5), "C:/reportes/reporte_ventas_20241128.csv", "Reporte de ventas del 25/11/2024 al 30/11/2024"),
        new ReporteGenerado(3, "Reporte_Ventas_20241125_1620", "PDF", LocalDateTime.now().minusDays(8), "C:/reportes/reporte_ventas_20241125.pdf", "Reporte de ventas del 20/11/2024 al 25/11/2024")
    );
    ...
    private void mostrarPrevisualizacionReporte(List<ReporteVentaDTO> datos, boolean permitirDescarga) {
        try {
            ...
            Label estado = new Label("Estado: Generado el " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")));
    ...
    private VBox generarContenidoReporteCompleto(List<ReporteVentaDTO> datos) {
        ...
        contenido.getChildren().addAll(tituloSeccion, tablaDatos, tituloGraficas, graficasBox, tituloEstadisticas, estadisticasBox);
    ...
    private PieChart crearGraficaPastelPreview(List<ReporteVentaDTO> datos) {
        ...
        // Agregar datos al gráfico de pastel (solo una vez)
        for (Map.Entry<String, Integer> entry : datosPorFormato.entrySet()) {
            pieChartPreview.getData().add(new PieChart.Data(entry.getKey() + " (" + entry.getValue() + ")", entry.getValue()));
        }
    ...
    private void exportarReporte(ExportStrategy strategy, String tipo) {
        try {
            ...
            if (archivo != null) {
                ...
                infoExtra.put("subtitulo", "Reporte generado el " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")));
